...
const NUM_OF_PAIRS = 1000;

function getProductOfThreeLargestCircuits(circuits) {
  const sorted = circuits
    .sort((a, b) => b.length - a.length)
    .map((circuit) => circuit.sort());
...
function groupCircuits(distances) {
...
async function main() {
  try {
    ...
    const circuits = groupCircuits(distances);
    // console.log('circuits:', circuits);
    const result = getProductOfThreeLargestCircuits(circuits);
    console.log({ result });
  } catch (err) {
    console.error(err);
  }
